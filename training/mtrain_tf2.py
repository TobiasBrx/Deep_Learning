...
class Trainer(object):
    ...
    with strategy.scope() as strategy:
        ...
        def __init__(self, train, model, im_size, label_size, 
                     n_epochs, batch_size,dims_d, do_mask=False, mask_h=None, mask_w=None):
            self.train_m = train
            self.label_size=label_size
            self.dims_d=dims_d
            self.x_slice=dims_d['x']
            self.y_slice=dims_d['y']
            self.z_slice=dims_d['z']
            ...
            try:
                if model =='FF':
                    self.model = MRI(train=self.train_m, im_size=im_size, label_size=label_size)
                elif model=='CNN':
                     self.model = MRI_2dCNN(train=self.train_m, im_size=im_size, label_size=label_size)
        ...
        def run(self, train_filenames, val_filenames, x_slice, y_slice, z_slice, strategy=strategy):
                ...
                _parse_function, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(
                    buffer_size=5).batch(self.batch_size)#.repeat()
                ...
                for epoch in range(self.n_epochs):
                    t_start = time.time()
                    self.train(epoch,x_slice,y_slice,z_slice,t_start)# x_slice, y_slice, z_slice, t_start)
                    final_accuracy = self.validate(self.x_slice, self.y_slice, self.z_slice).numpy()
        ...
        def train(self, epoch, x_slice, y_slice, z_slice, t_start):
            ...
            n_batches = int(total_training_files/self.batch_size)
            batch = 0
            avg_cost = 0.0
            avg_accuracy = 0.0
            times = []
            ...
            for x_batch, y_batch, _name in self.dataset:
                ...
                print(print_str)
                print('\rEpoch {0:>3} {1} {2:3.0f}% Cost: {3:6.4f}, Avg. Acc: {4:6.4f}, Current Acc: {5:.2f}, elapsed: {6:.0f}s, current: {7:.2f}'.format('#' + str(epoch + 1), print_str, completion * 100, avg_cost / (batch), avg_accuracy / (batch), batch_accuracy, epoch_time, batch_time))
        ...
        def train_step(self, x_batch, y_batch, _name, x_slice, y_slice, z_slice, batch, epoch, n_batches):
            ...
            ##### new line
            if self.do_mask:
                x_batch=tf.boolean_mask(x_batch, self.do_mask,axis=1).reshape(self.batch_size,self.mask_h,self.mask_w)
        ...
        def validate(self, x_slice, y_slice, z_slice):
            n_batches = int(total_validation_files/self.batch_size)
            batch = 0
            avg_accuracy = 0.0
