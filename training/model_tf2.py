...
class MRI:
    ...
    def __init__(self, name='MRI', train=True, im_size=156, label_size=3):
        self.name = name
...
class MRI_2dCNN:
    ...
    def __init__(self, name='MRI_2dCNN', train=True, im_size=156, label_size=3):
        ...
        shape_flat = int(shape2[-1] * self.im_size_squared / 4) #due to pooling
        shape3 = [shape_flat, 512]
        self.w_fc1 = tf.Variable(
            tf.random.truncated_normal(shape=shape3, dtype=tf.float32, stddev=0.1), name='w3_{0}'.format(name))
        self.b_fc1 =  tf.Variable(tf.constant(0.0, shape=shape3[-1:], dtype=tf.float32), name='b3_{0}'.format(name))
        
        shape4 = [512, self.output_size]
        self.w_fc2 = tf.Variable(
    ...
    def compute_output(self, X):
        ...
        flatten = tf.reshape(max_pool1, [max_pool1.shape[0], -1])
