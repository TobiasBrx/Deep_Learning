...
import numpy as np
from model import im_size, modified_size


def _parse_function(example_proto):
    features = {"image": tf.FixedLenFeature([im_size, im_size, im_size], tf.float32),
              "label": tf.FixedLenFeature((), tf.int64),
               'name': tf.FixedLenFeature((), tf.string, default_value='')}
    parsed_features = tf.parse_single_example(example_proto, features)
    cropsize = (im_size - modified_size) // 2
    parsed_features["image"]=parsed_features["image"][cropsize:im_size-cropsize, cropsize:im_size-cropsize, cropsize:im_size-cropsize]
